---
permalink: 2026/0527.html
title: 丰巢寄件系统基于DDD服务拆分的落地实践
date: 2026-05-27 09:00:00
tags: DDD
cover: https://technotes.oss-cn-shenzhen.aliyuncs.com/2026/202605281523403.jpg
thumbnail: https://technotes.oss-cn-shenzhen.aliyuncs.com/2026/202605281523403.jpg
categories: technotes
toc: true
description: 寄件系统面临高可用性、功能耦合度高、开发难度大等挑战。从数据层面看，所有数据表都放在一个数据库里，与服务耦合严重。基于以上原因，需要对寄件服务和数据表进行拆分。
---

## 一、背景

### 1.1 为什么要做服务拆分？

下面通过3个不同的视角来看待这个问题。

第一，系统视角。

- 提升服务的高可用性/性能：当系统面临可用性或性能问题时，通常需要将业务流程拆分，单独解决可用性和性能瓶颈。
- 降低功能耦合度：当系统内各功能间耦合度较高时，某个功能异常往往会波及到其他功能。为了解决这类问题，服务拆分势在必行。
- 提升系统资源收益：当服务体量过大时，运行需要消耗大量系统资源。为了满足系统运行需求，纵向增加资源的边际收益递减，横向进行服务拆分显得十分必要。

第二，业务视角。

- 满足未来的业务增长：假设每日寄件订单量为 1000 万，且按每年 30% 的速度增长。为满足业务增长需求，分而治之的策略十分必要。
- 提升迭代的敏捷性：业务方希望快速上线新需求，减少开发工时，这就要求服务必须轻量化。

第三，开发视角。

- 便于开发和维护代码：重量级服务给功能开发和代码维护带来了诸多阻碍，服务轻量化成为必然选择。

### 1.2 寄件系统现状及目标

**当前面临的问题：**

寄件系统目前主要面临以下几类问题：

- 高可用性挑战：服务体量庞大，核心与非核心场景耦合在同一服务中，给业务带来可用性隐患。
- 功能耦合度高：代码改动牵一发而动全身，往往涉及多个业务场景，存在研发质量隐患。
- 开发难度大：编译启动10min+，单元测试难度大【研发效率隐患】

另一方面，从数据层面看，寄件系统有 200张数据表，且都放在一个数据库里。数据表与服务耦合严重，由于功能依赖于数据，要想实现功能的解耦，数据层也需要进行拆分。

基于以上分析，对寄件服务和数据表进行拆分已成为必然选择。

**建设目标：**

- 提高寄件系统可用性，减少业务损失
- 提升业务迭代敏捷性，降低开发工时
- 提高研发质量，降低缺陷率

## 二、拆分方案

### 2.1 寄件流程概览

寄件业务流程完整生命周期如下图所示。

![](https://technotes.oss-cn-shenzhen.aliyuncs.com/2026/202605281529615.png)

第一步：寄件下单

![](https://technotes.oss-cn-shenzhen.aliyuncs.com/2026/202605272143880.png)

第二步：投递入柜

![](https://technotes.oss-cn-shenzhen.aliyuncs.com/2026/202605272144391.png)

第三步：快递员揽收

![](https://technotes.oss-cn-shenzhen.aliyuncs.com/2026/202605272144106.png)

### 2.2 场景分析

我们从三个维度进行场景分析：

- 业务流：站在用户视角，梳理完整的业务流程
- 能力：围绕业务流程，列举系统需要提供的能力
- 事件：围绕业务流程，列举过程中产生的系统事件

**场景一：寄件下单**

![](https://technotes.oss-cn-shenzhen.aliyuncs.com/2026/202605272145039.png)

**场景二：投递入柜**

![](https://technotes.oss-cn-shenzhen.aliyuncs.com/2026/202605272146677.png)

**场景三：快递员揽收**

![](https://technotes.oss-cn-shenzhen.aliyuncs.com/2026/202605272147187.png)

### 2.3 寄件领域模型

对上述3个场景进行汇总。

- 绿色：业务核心（实体/业务动作）
- 蓝色：相关能力（接口服务）
- 红色：业务事件

![](https://technotes.oss-cn-shenzhen.aliyuncs.com/2026/202605272148224.png)

汇总分析后，形成寄件领域模型：

![](https://technotes.oss-cn-shenzhen.aliyuncs.com/2026/202605271808141.png)

### 2.4 服务拆分

服务拆分时主要考虑两点：**职责单一性**和**软件包大小**。基于此，将寄件系统拆分为四个服务：

- 下单服务
- 投递服务
- 揽收服务
- 基础数据服务（以下简称基础服务）

**下单服务能力范围：**

| 类别     | （主要）接口/能力                                            |
| -------- | ------------------------------------------------------------ |
| 用户     | 查询寄件人实名、查询寄件人登录手机号、实名认证               |
| 快递公司 | 查询快递公司列表                                             |
| 订单     | 下单、查询订单、查询订单详情、修改订单                       |
| 支付     | 预锁格口支付、上门件支付、超重支付、微信支付分支付、支付回调 |
| 寄件码   | 生成寄件码                                                   |
| 地址     | 保存地址                                                     |
| 柜机     | 查看附近柜机、预锁格口、更换格口                             |
| 其它场景 | 取消订单、用户评价                                           |

**投递服务能力范围：**

| 类别     | （主要）接口/能力                                            |
| -------- | ------------------------------------------------------------ |
| 寄件码   | 查询寄件码（查下单服务）                                     |
| 柜机     | 查询可用格口                                                 |
| 支付     | 查询寄件钱包、查询优惠券、柜机支付、支付回调、柜机支付查询、退款、退款回调 |
| 运单     | 运力下单、接收路由、路由注册                                 |
| 订单     | 取消寄件、核验手机尾号、转单                                 |
| 其它场景 | 用户取消投递、用户柜机取消、用户取返                         |

**揽收服务能力范围：**

| 类别     | （主要）接口/能力                                    |
| -------- | ---------------------------------------------------- |
| 订单     | 查询待揽收列表、核验手机尾号、取消收件、接收巴枪揽收 |
| 运单     | 接收运费/重量                                        |
| 支付     | KA支付推送                                           |
| 结算     | 结算                                                 |
| 其它场景 | 快递员返件、快递员取消返件                           |

**基础服务能力范围：**

| 类别     | （主要）数据表     |
| -------- | ------------------ |
| 区域     | 省市区表、字典表   |
| 基础能力 | 熔断表、重试任务表 |

### 2.5 数据层拆分

**AS-IS（拆分前）**

![](https://technotes.oss-cn-shenzhen.aliyuncs.com/2026/202605272151835.png)

**TO-BE（拆分后）**

![](https://technotes.oss-cn-shenzhen.aliyuncs.com/2026/202605272153362.png)

## 三、架构设计

### 3.1 业务架构

![](https://technotes.oss-cn-shenzhen.aliyuncs.com/2026/202605271942006.png)

### 3.2 应用架构

![](https://technotes.oss-cn-shenzhen.aliyuncs.com/2026/202605280943469.png)

## 四、工程代码落地

### 4.1 工程清单

```markdown
# 下单服务
order-service
  |- order-service-api           rpc接口定义
  |- order-service-app-server    web 模块,http接口接入层
  |- order-service-biz           biz 模块,存放核心业务逻辑
  |- order-service-dao           数据处理层

# 投递服务
post-service
  |- post-service-api           rpc接口定义
  |- post-service-app-server    web 模块,http接口接入层
  |- post-service-biz           biz 模块,存放核心业务逻辑
  |- post-service-dao           数据处理层

# 揽收服务
pick-service
  |- pick-service-api           rpc接口定义
  |- pick-service-app-server    web 模块,http接口接入层
  |- pick-service-biz           biz 模块,存放核心业务逻辑
  |- pick-service-dao           数据处理层

# 基础服务
basic-service
  |- basic-service-api           rpc接口定义
  |- basic-service-app-server    web 模块,http接口接入层
  |- basic-service-biz           biz 模块,存放核心业务逻辑
  |- basic-service-dao           数据处理层
```

分层结构：

```
{服务名}
  |- {服务名}-api                   [rpc接口定义]
      |- enums       枚举类
      |- event       消息事件
      |- facade      Dubbo接口
      |- model       接口请求响应对象
  |- {服务名}-app-server            [web 模块,http接口接入层]
      |- async       消息处理
      |- rpc         Dubbo接口实现
      |- web         Web接口实现
  |- {服务名}-biz                   [biz 模块，存放核心业务逻辑]
      |- bo          业务对象
      |- enums       枚举类
      |- exception   异常类
      |- remote      外部http远程调用
      |- service     业务实现
      |- util        工具类
  |- {服务名}-dao                   [数据处理层]
      |- entity      实体类
      |- mapper      MyBatis Mapper
      |- util        工具类
```

**工程依赖关系：**

![](https://technotes.oss-cn-shenzhen.aliyuncs.com/2026/202605272223611.png)

### 4.2 通用功能包

```
send-common
  |- send-common-app      应用通用包
  |- send-common-db       db模块通用包
  |- send-common-test     单元测试工具包
```

**send-common-app 提供的特性：**

- 字典表缓存、渠道表缓存、区域表缓存
- 参数检验过滤器（RequestValidationFilter）
- 接口访问日志（Http/Dubbo）
- 多数据源（MySQL/Redis）

**send-common-db 提供的特性：**

- 动态切换数据源
- 数据加解密
- 影子库

### 4.3 使用示例

**示例一：多数据源配置**

```properties
#主库
send.main.datasource.url=jdbc:mysql://localhost:3306/send?useUnicode=true&characterEncoding=UTF-8&zeroDateTimeBehavior=convertToNull&allowMultiQueries=true&useSSL=false
send.main.datasource.username=root
send.main.datasource.password=[ENCRYPT]974UEpo2O6+KF79hjq7JCu9D+6OM/hk2
#备库1
send.backup.datasource.url=jdbc:mysql://localhost:3306/send?useUnicode=true&characterEncoding=UTF-8&zeroDateTimeBehavior=convertToNull&allowMultiQueries=true&useSSL=false
send.backup.datasource.username=root
send.backup.datasource.password=[ENCRYPT]974UEpo2O6+KF79hjq7JCu9D+6OM/hk2
#备库2
send.report.datasource.url=jdbc:mysql://localhost:3306/send?useUnicode=true&characterEncoding=UTF-8&zeroDateTimeBehavior=convertToNull&allowMultiQueries=true&useSSL=false
send.report.datasource.username=root
send.report.datasource.password=[ENCRYPT]974UEpo2O6+KF79hjq7JCu9D+6OM/hk2
#影子库
send.shadow.datasource.url=jdbc:mysql://localhost:3306/send?useUnicode=true&characterEncoding=UTF-8&zeroDateTimeBehavior=convertToNull&allowMultiQueries=true&useSSL=false
send.shadow.datasource.username=root
send.shadow.datasource.password=[ENCRYPT]974UEpo2O6+KF79hjq7JCu9D+6OM/hk2
```

**示例二：动态切换数据源**

```java
@Test
public void query_switchDataSource() {
    DictionaryEntity dictionary = SwitchDataSourceHelper.start(DynamicDataSourceType.SOURCE_SALVE_01)
            .execute(() -> dictionaryMapper.selectByPrimaryKey(2));
    Assert.assertEquals("17:00:00", dictionary.getDictValue());
}
```

**示例三：单元测试**

- 可通过配置 `spring.profiles.active=dev` 来运行 dev 环境的单元测试；
- `@SpringBootTest` 中的配置项可覆盖 Disconf 的配置，下面示例中切换了备库的数据源；

```java
@RunWith(SpringRunner.class)
@SpringBootTest(properties = {
        // 这里的配置项可覆盖 Disconf 的配置
        "spring.profiles.active=uat6",
        // 将备库地址换成其他实例
        "send.backup.datasource.url=jdbc:mysql://localhost:3306/send?useUnicode=true&characterEncoding=UTF-8&zeroDateTimeBehavior=convertToNull&allowMultiQueries=true&useSSL=false",
        "send.backup.datasource.username=root",
        "send.backup.datasource.password=[ENCRYPT]974UEpo2O6+KF79hjq7JCu9D+6OM/hk2"
})
public class SendDictionaryServiceImplTest {

    @Autowired
    private FcboxSendDictionaryMapper dictionaryMapper;

    @Test
    public void query_switchDataSource() {
        FcboxSendDictionary dictionary = SwitchDataSourceHelper.start(DynamicDataSourceType.SOURCE_SALVE_01)
                .execute(() -> dictionaryMapper.selectByPrimaryKey(2));
        Assert.assertEquals("17:00:00", dictionary.getDictValue());
    }
}
```

![](https://technotes.oss-cn-shenzhen.aliyuncs.com/2023/202303052135542.gif)

## 封面

![](https://technotes.oss-cn-shenzhen.aliyuncs.com/2026/202605281523403.jpg)

## 相关文章

- [EasyKafka：让消息收发更简单](https://mp.weixin.qq.com/s/ZtwQNmAZWQXwvZYMuCYSDQ)
- [04期：领域驱动设计与微服务](https://mp.weixin.qq.com/s/z2DkBrGwtg_Otn_OfUm9Vw)

